...
with GOLD_TAB:
    ...
    if st.session_state.get("booking_success_family_gold"):
        ...
        expander_functions.info_expander("family_gold_map", "family_gold_segment")
        ...
        if family_gold_form_submitted:
            if not FIRST_NAME or not LAST_NAME or not MOBILE_NUMBER or not EMAIL:
                ...
                show_family_gold_confirm_dialog(FIRST_NAME.strip(), LAST_NAME.strip(), "".join(MOBILE_NUMBER.split()), EMAIL.strip(), FORM_CATEGORY, EVENT_ORDER_ID, FORM_TICKET_TYPE, AVAILABLE_TICKET_FILTER_FORMULA)
...
with PLATINUM_TAB:
    ...
    if st.session_state.get("booking_success_family_platinum"):
        ...
        with st.form("family_platinum_form", clear_on_submit=False, enter_to_submit=False):
            ...
            MOBILE_NUMBER = st.text_input("Mobile Number (All countries supported!)", placeholder="Enter your mobile number (e.g.: +447xxxxxxxxx)", icon=":material/call:", help="Please enter the correct mobile number in the provided format without spaces.", key=f"family_platinum_mobile_number_{st.session_state.get('family_platinum_counter_mobile_number', 0)}")
        ...
        expander_functions.info_expander("family_platinum_map", "family_platinum_segment")
        ...
        if family_platinum_form_submitted:
            if not FIRST_NAME or not LAST_NAME or not MOBILE_NUMBER or not EMAIL:
                ...
                show_family_platinum_confirm_dialog(FIRST_NAME.strip(), LAST_NAME.strip(), "".join(MOBILE_NUMBER.split()), EMAIL.strip(), FORM_CATEGORY, EVENT_ORDER_ID, FORM_TICKET_TYPE, AVAILABLE_TICKET_FILTER_FORMULA)
...
with DIAMOND_TAB:
    ...
    if st.session_state.get("booking_success_family_diamond"):
        ...
        with st.form("family_diamond_form", clear_on_submit=False, enter_to_submit=False):
            ...
            MOBILE_NUMBER = st.text_input("Mobile Number (All countries supported!)", placeholder="Enter your mobile number (e.g.: +447xxxxxxxxx)", icon=":material/call:", help="Please enter the correct mobile number in the provided format without spaces.", key=f"family_diamond_mobile_number_{st.session_state.get('family_diamond_counter_mobile_number', 0)}")
        ...
        expander_functions.info_expander("family_diamond_map", "family_diamond_segment")
        ...
        if family_diamond_form_submitted:
            if not FIRST_NAME or not LAST_NAME or not MOBILE_NUMBER or not EMAIL:
                ...
                show_family_diamond_confirm_dialog(FIRST_NAME.strip(), LAST_NAME.strip(), "".join(MOBILE_NUMBER.split()), EMAIL.strip(), FORM_CATEGORY, EVENT_ORDER_ID, FORM_TICKET_TYPE, AVAILABLE_TICKET_FILTER_FORMULA)
